...
class IntentVector:
    ...
    def is_blocking(self) -> bool:
        return (
            ...
            or self.risk >= 80
            or self.manipulation_risk >= 70
            or self.reality_score <= 40
            or self.entropy_score >= 85
        )
...
def defensive_vector(servo: ServoName, symbol: str, reason: str, cycle_id: str | None = None) -> IntentVector:
    return IntentVector(
...
def block_vector(servo: ServoName, symbol: str, reason: str, cycle_id: str | None = None) -> IntentVector:
    return IntentVector(
        cycle_id=cycle_id or new_cycle_id(),
        servo=servo,
        symbol=symbol,
