...
class RegimeDetector:
    def detect(self, vectors: Iterable[IntentVector]) -> str:
        ...
        if not vectors: return 'SYSTEMIC_RISK'
        ...
        if avg_manipulation >= 70: return 'MANIPULATED'
        if avg_entropy >= 80: return 'CHAOTIC'
        if avg_risk >= 80: return 'SYSTEMIC_RISK'
        if avg_entropy >= 65 and avg_confidence <= 45: return 'WHITE_NOISE'
        if avg_confidence >= 70 and avg_evidence >= 65: return 'TRENDING'
        if avg_risk >= 60: return 'VOLATILE'
        return 'NORMAL'
