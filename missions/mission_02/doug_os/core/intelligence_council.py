...
class IntelligenceCouncil:
    ...
    def decide(self, vectors: Iterable[IntentVector], cycle_id: str | None = None) -> dict:
        vectors = list(vectors)
        valid_vectors: list[IntentVector] = []
        rejected_vectors: list[dict] = []
        if cycle_id is None:
            first_valid = next((v for v in vectors if isinstance(v, IntentVector)), None)
            cycle_id = first_valid.cycle_id if first_valid else str(uuid4())
        ...
        if not valid_vectors:
            return {
                "cycle_id": cycle_id,
                "decision": "BLOCK",
                "reason": "no_valid_vectors_for_cycle",
                "confidence": 0,
                "regime": "SYSTEMIC_RISK",
                "weights": {},
                "accepted": [],
                "rejected": rejected_vectors,
            }
        ...
        for vector in valid_vectors:
            if vector.servo == "risk_empire" and vector.is_blocking():
                return {
                    ...
                    "regime": "SYSTEMIC_RISK",
                    "weights": {},
                    "accepted": [v.to_dict() for v in valid_vectors],
                    "rejected": rejected_vectors,
                }
